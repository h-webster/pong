...
function moveTowards(pos) {
    let dy = abs((bot.y + bot.h / 2) - pos);

    if (dy <= SPEED) {
        return;
    }

    if (pos > bot.y + bot.h / 2) {
        bot.Move(SPEED);
    } else if (pos < bot.y + bot.h / 2) {
        bot.Move(-SPEED);
    }
}

function predictLocation() {
    let fakeBall = new Ball(ball.x, ball.y, ball.r, ball.vx, ball.vy);
    while (fakeBall.x < bot.x) {
        fakeBall.Simulate();
    }
    return fakeBall.y;
}
